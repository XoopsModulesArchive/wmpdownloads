...
$GLOBALS['xoopsOption']['template_main'] = 'wmpdownloads_topten.html';
...
$arr = [];
$result = $xoopsDB->query('SELECT cid, title FROM ' . $xoopsDB->prefix('wmpdownloads_cat') . ' WHERE pid=0');
$e = 0;
$rankings = [];
while (list($cid, $ctitle) = $xoopsDB->fetchRow($result)) {
    $rankings[$e]['title'] = sprintf(_MD_TOP10, htmlspecialchars($ctitle, ENT_QUOTES | ENT_HTML5));

    $query = 'SELECT lid, cid, title, hits, rating, votes FROM ' . $xoopsDB->prefix('wmpdownloads_downloads') . " WHERE status>0 AND (cid=$cid";
