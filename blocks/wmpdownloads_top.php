<?php
// $Id: wmpdownloads_top.php,v 1.8 2003/03/28 04:04:50 w4z004 Exp $
//  ------------------------------------------------------------------------ //
...
/******************************************************************************
 * Function: b_wmpdownloads_top_show
 ...
 * @return array
 */
function b_wmpdownloads_top_show($options)
{
    global $xoopsDB;
    ...
    $result = $xoopsDB->query('SELECT lid, cid, title, date, hits FROM ' . $xoopsDB->prefix('wmpdownloads_downloads') . ' WHERE status>0 ORDER BY ' . $options[0] . ' DESC', $options[1], 0);
...
function b_wmpdownloads_top_edit($options)
{
    $form = '' . _MB_wmpdownloadS_DISP . '&nbsp;';
    ...
    $form .= '<input type="text" name="options[]" value="' . $options[1] . '">&nbsp;' . _MB_wmpdownloadS_FILES . '';

    $form .= '&nbsp;<br>' . _MB_wmpdownloadS_CHARS . "&nbsp;<input type='text' name='options[]' value='" . $options[2] . "'>&nbsp;" . _MB_wmpdownloadS_LENGTH . '';
