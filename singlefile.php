...
$lid = (int)$_GET['lid'];
$cid = (int)$_GET['cid'];
$GLOBALS['xoopsOption']['template_main'] = 'wmpdownloads_singlefile.html';
require XOOPS_ROOT_PATH . '/header.php';
$q = 'SELECT d.lid, d.cid, d.title, d.url, d.homepage, d.version, d.size, d.platform, d.logourl, d.status, d.date, d.hits, d.rating, d.votes, d.comments, t.description FROM ' . $xoopsDB->prefix('wmpdownloads_downloads') . ' d, ' . $xoopsDB->prefix('wmpdownloads_text') . " t WHERE d.lid=$lid AND d.lid=t.lid AND status>0";
$result = $xoopsDB->query($q);
[$lid, $cid, $title, $url, $homepage, $version, $size, $platform, $logourl, $status, $time, $hits, $rating, $votes, $comments, $description] = $xoopsDB->fetchRow($result);
...
$xoopsTpl->assign('file', ['id' => $lid, 'cid' => $cid, 'rating' => $rating, 'title' => $dtitle . $new . $pop, 'logourl' => $logourl, 'updated' => $datetime, 'description' => $description, 'adminlink' => $adminlink, 'hits' => $hits, 'votes' => $votestring, 'platform' => $platform, 'comments' => $comments, 'size' => $size, 'homepage' => $homepage, 'version' => $version, 'category' => $path, 'lang_dltimes' => sprintf(_MD_DLTIMES, $hits), 'mail_subject' => rawurlencode(sprintf(_MD_INTFILEFOUND, $xoopsConfig['sitename'])), 'mail_body' => rawurlencode(sprintf(_MD_INTFILEFOUND, $xoopsConfig['sitename']) . ':  ' . XOOPS_URL . '/modules/wmpdownloads/singlefile.php?cid=' . $cid . '&amp;lid=' . $lid)]);
