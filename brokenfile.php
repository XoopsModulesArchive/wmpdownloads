...
if (!empty($_POST['submit'])) {
    ...
    if (0 != $sender) {
        ...
        $result = $xoopsDB->query('SELECT COUNT(*) FROM ' . $xoopsDB->prefix('wmpdownloads_broken') . " WHERE lid=$lid AND sender=$sender");
        ...
        $result = $xoopsDB->query('SELECT COUNT(*) FROM ' . $xoopsDB->prefix('wmpdownloads_broken') . " WHERE lid=$lid AND ip = '$ip'");
    ...
    $newid = $xoopsDB->genId($xoopsDB->prefix('wmpdownloads_broken') . '_reportid_seq');

    $sql = sprintf("INSERT INTO %s (reportid, lid, sender, ip) VALUES (%u, %u, %u, '%s')", $xoopsDB->prefix('wmpdownloads_broken'), $newid, $lid, $sender, $ip);
    ...
    $GLOBALS['xoopsOption']['template_main'] = 'wmpdownloads_brokenfile.html';
    require XOOPS_ROOT_PATH . '/header.php';
    $xoopsTpl->assign('lang_reportbroken', _MD_REPORTBROKEN);
    $xoopsTpl->assign('file_id', (int)$_GET['lid']);
