...
{
    echo '<br><br><p><a href="' . XOOPS_URL . '/modules/wmpdownloads/index.php"><img src="' . XOOPS_URL . '/modules/wmpdownloads/images/logo-en.gif" border="0" alt""></a></p><br><br>';
}
...
function newdownloadgraphic($time, $status)
{
    ...
    if ($startdate < $time) {
        if (1 == $status) {
            $new = '&nbsp;<img src="' . XOOPS_URL . '/modules/wmpdownloads/images/newred.gif" alt="' . _MD_NEWTHISWEEK . '">';
        } elseif (2 == $status) {
            $new = '&nbsp;<img src="' . XOOPS_URL . '/modules/wmpdownloads/images/update.gif" alt="' . _MD_UPTHISWEEK . '">';
        }
    }
...
function popgraphic($hits)
{
    ...
    if ($hits >= $xoopsModuleConfig['popular']) {
        return '&nbsp;<img src ="' . XOOPS_URL . '/modules/wmpdownloads/images/pop.gif" alt="' . _MD_POPULAR . '">';
    }
...
function updaterating($sel_id)
{
    ...
    $query = 'select rating FROM ' . $xoopsDB->prefix('wmpdownloads_votedata') . ' WHERE lid = ' . $sel_id . '';
    ...
    $sql = sprintf('UPDATE %s SET rating = %u, votes = %u WHERE lid = %u', $xoopsDB->prefix('wmpdownloads_downloads'), $finalrating, $votesDB, $sel_id);
...
function getTotalItems($sel_id, $status = '')
{
    ...
    $query = 'select count(*) from ' . $xoopsDB->prefix('wmpdownloads_downloads') . ' where cid=' . $sel_id . '';
    ...
    for ($i = 0; $i < $size; $i++) {
        $query2 = 'select count(*) from ' . $xoopsDB->prefix('wmpdownloads_downloads') . ' where cid=' . $arr[$i] . '';
